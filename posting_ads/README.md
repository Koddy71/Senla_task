...
Микросервисы взаимодействуют синхронно через REST API, асинхронно через RabbitMQ
...
- Удаление пользователя и объявления выполняется логически: данные не удаляются физически, а меняется статус. При удалении пользователя, все его объявления также удаляются
...
- Логги можно посмотреть в /usr/local/tomcat/logs/{имя_сервиса}.log
